'''
getNoMetals.py
    Get the residues that have no metal contacts, for each of the three datasets
    
    In this code, these datasets are denoted as follows.

    "First": only Mg
    "Second": Mg & 5 most prevalent metals
    "Third": all metals

Usage: getNoMetals.py <pdbIDFilePath> <discrepDataDirPath> <resultFilePath> <maxDist> <metalNameList>
...
Parameters:
    ...
    <maxDist> - the maximum observed metal contact distance amongst all metals. This is found from the thirdBeyond dataset, which can be retrieved using getMetalDatasets.py.
    <metalNameList> - comma-separated list of metals.

Example 1: python3 metalAnalysis/code/getNoMetals.py results/standalone_01-04-22.txt results/standalone_01-04-22 metalAnalysis/results/firstNoMetals.txt 87.38854 MG

Example 2: python3 metalAnalysis/code/getNoMetals.py results/standalone_01-04-22.txt results/standalone_01-04-22 metalAnalysis/results/secondNoMetals.txt 87.38854 ZN,MG,CA,FE,NA,MN

Example 3: python3 metalAnalysis/code/getNoMetals.py results/standalone_01-04-22.txt results/standalone_01-04-22 metalAnalysis/results/thirdNoMetals.txt 87.38854 ZN,MG,CA,FE,NA,MN,K,CU,NI,CO,CD,HG,PT,MO,AL,V,BA,SR,RU,CS,W,YB,AU,Y,LI,PB,GD,TL,RB,SM,IR,PR,RH,EU,PD,AG,OS,LU,HO,TB,CR,GA,LA,SB,CE,ER,IN,BI,DY,BE,ZR,SN,HF,TA,RE,PA,U
...
def main(pdbIDFilePath: str, discrepDirPath: str, resultFilePath: str, maxDist: float, metalNameList: str) -> None:
    """
    ...
    """
    # Get the residue dictionary list.
    resDictList = getResDictList(metalNameList, pdbIDFilePath, maxDist)
    ...
    for resDict in resDictList:
        ...
        if residueID in discrepDict.keys():
            ...
            assert(resDict['metalDist'] == maxDist)

            discrepDict[residueID].update({'metal_distance':resDict['metalDist']})
...
def getResDictList(metalNameList: list, pdbIDFilePath: str, maxDist: float) -> list:
    '''
            ...
            if os.path.isfile(structFilePath):
                ...
                for res in atpResList:
                    ...
                    if minDist == sys.maxsize:
                        resDictList.append({'pdbid':pdbID, 'model':res.parent.parent.id, 'chain':res.parent.id, 'residue_name':res.resname, 'residue_number':res.id[1], 'metalDist':maxDist})
...
if __name__ == '__main__':
    try:
        _, pdbIDFilePath, discrepDirPath, resultFilePath, maxDistStr, metalNameList = sys.argv
        main(pdbIDFilePath, discrepDirPath, resultFilePath, float(maxDistStr), metalNameList.split(','))
